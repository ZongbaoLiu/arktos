...
// EGatewaysGetter has a method to return a EGatewayInterface.
// A group's client should implement this interface.
type EGatewaysGetter interface {
	EGateways(namespace string) EGatewayInterface
	EGatewaysWithMultiTenancy(namespace string, tenant string) EGatewayInterface
}

// EGatewayInterface has methods to work with EGateway resources.
type EGatewayInterface interface {
	...
	List(opts metav1.ListOptions) (*v1.EGatewayList, error)
	Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface
	Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.EGateway, err error)
	EGatewayExpansion
}

// eGateways implements EGatewayInterface
type eGateways struct {
...
// newEGateways returns a EGateways
func newEGateways(c *CloudgatewayV1Client, namespace string) *eGateways {
	return newEGatewaysWithMultiTenancy(c, namespace, "system")
}

func newEGatewaysWithMultiTenancy(c *CloudgatewayV1Client, namespace string, tenant string) *eGateways {
	return &eGateways{
		client:  c.RESTClient(),
...
func (c *eGateways) Get(name string, options metav1.GetOptions) (result *v1.EGateway, err error) {
	result = &v1.EGateway{}
	err = c.client.Get().
		Tenant(c.te).
		Namespace(c.ns).
...
func (c *eGateways) List(opts metav1.ListOptions) (result *v1.EGatewayList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &v1.EGatewayList{}
	...
	if wgLen > 1 {
		...
		wg.Add(wgLen)
		results := make(map[int]*v1.EGatewayList)
		errs := make(map[int]error)
		for i, client := range c.clients {
			go func(c *eGateways, ci rest.Interface, opts metav1.ListOptions, lock *sync.Mutex, pos int, resultMap map[int]*v1.EGatewayList, errMap map[int]error) {
				r := &v1.EGatewayList{}
				err := ci.Get().
		...
		itemsMap := make(map[string]v1.EGateway)
		for j := 0; j < wgLen; j++ {
...
// Watch returns a watch.Interface that watches the requested eGateways.
func (c *eGateways) Watch(opts metav1.ListOptions) watch.AggregatedWatchInterface {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
...
func (c *eGateways) Create(eGateway *v1.EGateway) (result *v1.EGateway, err error) {
	result = &v1.EGateway{}
...
func (c *eGateways) Update(eGateway *v1.EGateway) (result *v1.EGateway, err error) {
	result = &v1.EGateway{}
...
func (c *eGateways) Delete(name string, options *metav1.DeleteOptions) error {
...
func (c *eGateways) DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error {
	var timeout time.Duration
...
func (c *eGateways) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.EGateway, err error) {
	result = &v1.EGateway{}
